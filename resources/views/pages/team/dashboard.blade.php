...
<x-layouts.dashboard-team title="Dashboard Tim {{ $team?->name }}">
    ...
    <div class="alert alert-danger">
        ...
        <strong>Pendaftaran Tim Ditolak.</strong><br>

        @if (!empty($payment?->reason))
        <p><strong>Alasan Penolakan:</strong> {{ $payment->reason }}</p>
        @else
        <p>Tim Anda ditolak oleh admin karena tidak memenuhi ketentuan.</p>
        @endif

        <p>
            Silakan buka <strong>email akun terdaftar</strong> untuk melakukan
            <strong>registrasi ulang</strong>.
        </p>

        <a
            href="{{ 'https://api.whatsapp.com/send/?phone='.$appSettings['phone'].'&text=Halo, saya '.($leader?->name ?? 'Peserta').' dari tim '.($team?->name ?? '-').'. Saya ingin menanyakan alasan penolakan tim saya.' }}"
            target="_blank"
            class="alert-link">
            Hubungi Admin
        </a>
